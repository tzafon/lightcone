...
class Lightcone(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        This automatically infers the `api_key` argument from the `LIGHTCONE_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("LIGHTCONE_API_KEY")
        if api_key is None:
            raise LightconeError(
                "The api_key client option must be set either by passing api_key to the client or by setting the LIGHTCONE_API_KEY environment variable"
            )
        ...
        if base_url is None:
            base_url = f"https://api.tzafon.ai/v1"
